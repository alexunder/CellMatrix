...
void *handle_client_message(void *opaque)
{
	...
	*ret_len = 0;
	...
	case WAIT_FOR_EVENT: {
		...
		ptr = (char *)buf;
		ALOGV("wifi_wait_for_event(%s), len:%d", iface, event_len);
	...
	case COMMAND: {
		...
		if (reply_len == 0 || reply_len > (WIFI_MAX_RPLY_LEN - 12)) {
			*reply = -1;
			ALOGE("reply_len %u exceeds maximum allowed: %u",
			      reply_len, WIFI_MAX_RPLY_LEN);
			break;
		}
		...
		reply_buf = buf;
		ALOGV("wifi_command(%s): '%s' (len:%d)", iface, command, reply_len);
	...
	case CHANGE_FW_PATH:
		ALOGE("changing firmware path currently unsupported");
		*reply - 1;
		break;
	case ENSURE_ENTROPY_FILE:
		ALOGE("ensure entroy file currently unsupported");
		*reply - 1;
		break;
	default:
