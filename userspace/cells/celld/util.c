...
int walkdir(void *ctx, const char *path, int depth,
	    void (*callback)(void *ctx, const char *path, struct dirent *e))
{
	int ret;
	DIR *dir;
	struct dirent entry;
	     ...
	     ret = readdir_r(dir, &entry, &result))
	{
		...
		callback(ctx, full_name, &entry);
		if (depth && entry.d_type == DT_DIR) {
			ret = walkdir(ctx, full_name, depth-1, callback);
			if (ret)
				break;
		}
	}
